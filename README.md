...
- V1.1 安装器支持覆盖升级，会在安装前停止旧版 MemGuard 相关进程，避免安装目录占用冲突。
...
- 启动时先执行一次温和清理。
- Codex 运行中默认只清理安全判定的残留工具链；无法确认的目标只报告为“需人工确认”。
...
- `MemGuard-Setup-V1.1.0.exe`：安装版
- `MemGuard-V1.1.0-x64.exe`：便携版
...
V1.1 安装器使用固定应用 GUID，并在安装前停止旧版 MemGuard 计划任务和 MemGuard 自身进程，避免版本迭代时出现安装冲突。
